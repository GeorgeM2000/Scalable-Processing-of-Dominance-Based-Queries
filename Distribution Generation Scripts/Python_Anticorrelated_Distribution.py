# -*- coding: utf-8 -*-
"""
Created on Sat Nov  4 00:12:09 2023
...
def generate_anticorrelated_distribution(num_points, num_dimensions, covariance=0.95):
...
def plot_2d_distribution(data):
    if data.shape[1] == 2:
        ...
        plt.xlabel('x')
        plt.ylabel('y')
        plt.title('Anticorrelated Distribution in 2D')
        plt.show()
    else:
...
def main():
    ...
    data = generate_anticorrelated_distribution(num_points, num_dimensions)
    dataframe = pd.DataFrame(data=data)

    file = f'Anticorrelated_Data_{num_dimensions}D.txt'
